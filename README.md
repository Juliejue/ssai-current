...
1. `COMMUNITY` 与 `seed()` 仍引用旧的上海 placeId。`placeById` 兜底是 `PLACES[0]`，**不崩但静默错配** —— 旧记录全显示成"水果店"。
2. 7 条真实引文（`p.quote`）存了但**一次都没被渲染**，没接进匿名到访流。
3. 卡片翻页圆点是 `list.map()` 生成的，地点从 8 变 26 后是一排 26 个点。
4. `ROUTES` 四条路线数据完整但 UI 没露出。
5. 地点的距离与交通是估算值，需核对。
...
- 仓库**保持 private**。内含 7 条真人原话（群聊摘录），公开发布前需当事人同意。
