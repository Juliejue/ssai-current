...
**在场证明 L1（FR-08）**：围栏判定整个在浏览器里完成，**用户的经纬度一次都不上传**——
只上传「进没进围栏」和「待了多久」。因此浏览器报的等级不可信，服务端只会往下降级
...
0. **外部凭据尚未配置** —— 逐步开通步骤见 `开通清单-外部服务.md`。四项都缺时页面仍然完整可走，只是语音按钮报错、距离是原型估算、围栏核验降级为手动确认、事件只进日志。
