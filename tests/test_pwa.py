...
def test_iteration_ui_has_four_cities_media_and_no_removed_home_copy():
    ...
    assert 'id="match-voice"' in html
    assert 'data-match="agents"' in html
...
def test_screenshot_regressions_keep_copy_and_controls_clean():
    html = (ROOT / "此在-current-原型.html").read_text(encoding="utf-8")
    settings = html[html.index("function renderSettings("):html.index("function renderBlocked(")]
    ...
    for removed in ("只打开你愿意给的", "存：结构化之后的需求", "不存：原话、语音", "清空这台设备上的全部记录", "我们的边界"):
